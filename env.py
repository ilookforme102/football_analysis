...
def get_matches(offset_number,conn,bulk):
    ...
    ON m.id = h.match_id 
    WHERE DATE(FROM_UNIXTIME(m.match_time)) >= DATE(NOW()) AND DATE(FROM_UNIXTIME(m.match_time)) <= DATE(NOW() + INTERVAL 1 DAY)
...
def count_matches():
    ...
    try:
        ...
        # SQL query
        sql = "SELECT COUNT(*) as match_number FROM `wpdbtt_api_matches` WHERE DATE(FROM_UNIXTIME(match_time)) >= DATE(NOW()) AND DATE(FROM_UNIXTIME(match_time)) <= DATE(NOW() + INTERVAL 1 DAY);"
